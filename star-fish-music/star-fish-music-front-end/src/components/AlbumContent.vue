<template>
  <div class="content">
    ...
    <ul>
      ...
      <div v-if="songList.length == 0" style="text-align: center; padding-top: 30px">
        <h3 >暂无数据，请联系管理员添加......</h3>
      </div>
      <li v-else class="list-content" v-for="(item, index) in songList" :key="index">
        <div class="song-item" :class="{'is-play': id === item.id}"  @click="toplay(item.id, item.songUrl, item.pic, index, item.songName, item.singerName, item.lyric)">
