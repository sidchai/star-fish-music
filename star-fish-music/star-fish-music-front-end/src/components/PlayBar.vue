  ...
  <div class="play-bar" :class="{show:!toggle}">
    ...
    <el-row>
      <div class="kongjian" >
        ...
        <!--歌曲图片-->
        <div class="item-img" @click="goPlayerPage(picUrl)">
          <img :src=picUrl alt="">
        </div>
        <!--播放进度-->
...
import { addCollection } from '@/api/mySong'
...
export default {
  ...
  methods: {
    ...
    goPlayerPage (picUrl) {
      if(picUrl === 'http://s4.music.126.net/style/web2/img/default/default_album.jpg') {
        return false
      }
      ...
      if (this.loginIn) {
        ...
        addCollection(params)
          .then(resp => {
            ...
            } else if (resp.code === 28009) {
              this.notify(resp.message, 'warning')
            } else {
              this.$notify.error({
                title: resp.message,
                showClose: false
