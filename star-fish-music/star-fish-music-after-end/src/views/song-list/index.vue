<template>
  <div
    ...
    <div class="filter-container">
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="17" :md="17" :lg="18">
          <el-button v-permission="'/song-list/add'" class="filter-item" type="primary" icon="el-icon-edit" @click="handleAdd">添加</el-button>
          <el-button v-permission="'/song-list/delete-all'" class="filter-item" type="danger" icon="el-icon-delete" @click="batchDelete">批量删除</el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <el-table :data="data" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="歌单图片" width="180" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="歌单标题" width="300" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.title }}</span>
        </template>
      </el-table-column>

      <el-table-column label="歌单描述" width="650" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="风格" width="100" align="center">
        <template slot-scope="scope">
          <el-tag type="success">{{ scope.row.style }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" min-width="250">
        <template slot-scope="scope">
          <el-button v-permission="'/song-list/edit'" type="primary" size="small" @click="handleEdit(scope.row)">编辑
          </el-button>
          <el-button v-permission="'/song-list/delete'" type="danger" size="small" @click="handleDelete(scope.row)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" :before-close="handleClose">
      <el-form ref="form" :model="form" :rules="rules" :status-icon="true">
        <el-form-item label="歌单图片" :label-width="formLabelWidth">
          <upload
            v-show="isShowUpload"
            :action="avatarUrl"
            :src="form.pic"
        ...
        <el-row :gutter="20">
          ...
          <el-col :xs="24" :md="24" :lg="12">
            <el-form-item label="歌单描述" :label-width="formLabelWidth" prop="introduction">
        ...
        <el-row :gutter="20">
          <el-col :xs="24" :md="24" :lg="12">
            <el-form-item label="歌单类型" :label-width="formLabelWidth" prop="style">
...
export default {
  ...
  data() {
    return {
      ...
      avatarUrl: process.env.VUE_APP_URL + 'music/oss/file/upload?model=img/songListPic', // 上传地址
      rules: { // 表单校验规则
        title: [
          { required: true, message: '歌单标题不能为空', trigger: 'blur' }
        ],
        style: [
          { required: true, message: '歌单风格不能为空', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    // 得到所有歌曲信息
    GetSongListInfo() {
      getSongListInfo().then(resp => {
        if (resp.code === 20000) {
          this.loading = false
          this.tableData = resp.data.songListInfo
          this.tempDate = resp.data.songListInfo
        }
      })
    },
    // 添加
    ...
    handleDelete(row) {
      this.$confirm('此操作将删除该歌单信息, 是否继续?', '提示', {
        ...
        deleteSongList(row.id).then(resp => {
          if (resp.code === 20000) {
            this.$message({
              type: 'success',
    ...
    batchDelete() {
      ...
      this.$confirm('此操作将删除选中歌曲信息, 是否继续?', '提示', {
        ...
        deleteBatchSongList(params).then(resp => {
    ...
    statusChange(row) {
      ...
      * */
      ...
      // 改变数据库中状态值
      changeSongListStatus(song).then(resp => {
        if (resp.code === 20000) {
    ...
    handleAvatarSuccess(resp) {
      if (resp.success) {
        ...
        // 修改或添加数据库中信息
        addOrChangeSongListAvatar(data).then(resp => {
        })
      } else {
